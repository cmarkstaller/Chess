...
public class MemoryAuthDao implements AuthDao {
    ...
    public void insertAuth(AuthData authObject) throws dataAccess.Exceptions.DataAccessException {
        if (hashMap.containsKey(authObject.authToken())) throw new dataAccess.Exceptions.DataAccessException("Duplicated Key error");
        hashMap.put(authObject.authToken(), authObject);
    }
    public AuthData getAuth(String authToken) throws NotLoggedInException {
        if (!hashMap.containsKey(authToken)) throw new NotLoggedInException("you are not logged in error");
